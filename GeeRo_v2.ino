...
// ============================================================
//  ROBOT - VL53L1X × 3 (Wire على 26,27)
//  Front=25pin XSHUT | Left=32 | Right=33
// ============================================================
#define I2C_SDA       26
#define I2C_SCL       27
#define TOF_XSHUT_F   25   // ← Front  (كان FRONT_TRIG)
#define TOF_XSHUT_L   32
#define TOF_XSHUT_R   33
#define TOF_ADDR_F  0x30
#define TOF_ADDR_L  0x31
...
#define CH_ELBOW       1
#define CH_GRIPPER     3   // PCA channel 3 (skip 2 - محذوف)
#define NUM_SERVOS     3   // ← Shoulder + Elbow + Gripper فقط
...
// index 0=Shoulder | 1=Elbow | 2=Gripper
...
// ============================================================
//  ROBOT - Sensors (Non-blocking) - 3x ToF
// ============================================================
void updateSensors() {
...
void updateTorqueManagement() {
  ...
  for (uint8_t i = 0; i < NUM_SERVOS; i++) {
    ...
    unsigned long holdTime = (servos[i].channel == CH_GRIPPER) ? GRIPPER_HOLD_MS : TORQUE_HOLD_MS;
    if (now - servos[i].lastMoveTime > holdTime) {
      disableTorque(i);
      Serial.printf("💤 Ch%d OFF (%.1f°)\n", servos[i].channel, servos[i].currentAngle);
    }
  }
}
...
// ============================================================
//  ARM - Control  (Shoulder=LY | Elbow=RY | Gripper=L2/R2)
// ============================================================
void runArmControl(ControllerPtr ctl) {
  unsigned long now = millis();

  // △ Triangle → Reset الذراع
  ...
  if (fLY != 0.0f) moveServo(0, -fLY * speed);   // index 0 = SHOULDER

  // Elbow    ← Right Stick Y
  if (fRY != 0.0f) moveServo(1, -fRY * speed);   // index 1 = ELBOW

  // Gripper  ← L2 قفل / R2 فتح
  float gripDelta = (fR2 - fL2) * speed;
  if (abs(gripDelta) > 0.015f) moveServo(2, gripDelta);  // index 2 = GRIPPER
}
...
// ============================================================
void initToF() {
  Serial.println("[TOF] Init 3x VL53L1X...");
...
void setup() {
  ...
  delay(10);
  resetAllServos();
  Serial.println("[ARM] 3 Servos ready at 90°");
...
void loop() {
  ...
  // Torque Management في المانيوال بس
  ...
  for (auto ctl : myControllers) {
    ...
    if (robotMode == 0) {
      runManual(ctl);      // تحكم الروبوت
      runArmControl(ctl);  // ✅ الذراع في المانيوال بس
    }
  }

  if (robotMode == 1) runAuto();

  // Debug كل 500ms - في المانيوال بس
  if (robotMode == 0) {
    static unsigned long lastPrint = 0;
    if (millis() - lastPrint > 500) {
      lastPrint = millis();
      Serial.printf("[ARM][%s x%.2f] SHO:%.1f° ELB:%.1f° GRP:%.1f°\n",
        precisionMode ? "PREC" : "NORM", speedMultiplier,
        servos[0].currentAngle, servos[1].currentAngle,
        servos[2].currentAngle);
    }
  }
}
