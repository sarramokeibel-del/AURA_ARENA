// ============================================================
//  ROBOT (Motors + Auto/Manual) + ARM (5 Servos via PCA9685)
...
#define WRIST_ROLL_MIN_US  600
#define WRIST_ROLL_MAX_US  2400
...
ServoConfig servos[NUM_SERVOS] = {
  ...
  { CH_WRIST_ROLL,   0, 180, WRIST_ROLL_MIN_US, WRIST_ROLL_MAX_US, 90.0f, 0, true },
...
void runArmControl(ControllerPtr ctl) {
  ...
  float fLX = applyArmDeadzone(ctl->axisX());
  ...
  if (fLX != 0.0f) moveServo(CH_WRIST_ROLL,   fLX * speed);
...
void loop() {
  ...
  if (robotMode == 0) {
    ...
    if (millis() - lastPrint > 500) {
      lastPrint = millis();
      Serial.printf("[ARM][%s x%.2f] SHO:%.1f° ELB:%.1f° WP:%.1f° WR:%.1f° GRP:%.1f°\n",
